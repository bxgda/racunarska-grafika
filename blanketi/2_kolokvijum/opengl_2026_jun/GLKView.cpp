...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	switch (nChar) {
		case 'A':
			m_glRenderer.angInner += offset;
			break;
		case 'S':
			m_glRenderer.angInner -= offset;
			break;
		case 'Q':
			m_glRenderer.angRing += offset;
			break;
		case 'W':
			m_glRenderer.angRing -= offset;
			break;
		case 'D':
			m_glRenderer.angSwing += offset;
			break;
		case 'F':
			m_glRenderer.angSwing -= offset;
			break;
...
// ovo nije trebalo po zadatku ali sam dodao radi lakseg resavanja

void CGLKView::OnMouseMove(UINT nFlags, CPoint point)
{
	if (nFlags & MK_LBUTTON)
	{
		float dx = (float)(point.x - m_lastMouse.x);
		float dy = (float)(point.y - m_lastMouse.y);

		m_glRenderer.cameraAlpha += dx * 0.005f;
		m_glRenderer.cameraBeta += dy * 0.005f;
...
{
	if (zDelta > 0)
		m_glRenderer.cameraDist -= 0.5f;
	else
		m_glRenderer.cameraDist += 0.5f;

	if (m_glRenderer.cameraDist < 2.0f)
		m_glRenderer.cameraDist = 2.0f;
	if (m_glRenderer.cameraDist > 50.0f)
		m_glRenderer.cameraDist = 50.0f;

...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{
	case 'G':
		if (m_glRenderer.brojDelova < 6)
			m_glRenderer.brojDelova += 1;
		break;
	case 'H':
		if (m_glRenderer.brojDelova > 4)
			m_glRenderer.brojDelova -= 1;
		break;
	case 'D':
	...
	case 'R':
		...
		break;
	case 'Q':
		if (m_glRenderer.ugao1 <= 20)
			m_glRenderer.ugao1 += 5;
		break;
	case 'W':
		if (m_glRenderer.ugao1 >= -10)
			m_glRenderer.ugao1 -= 5;
		break;
	case 'U':
		if (m_glRenderer.slide <= 0.1)
			m_glRenderer.slide += 0.025;
		break;
	case 'I':
		if (m_glRenderer.slide >= -0.1)
			m_glRenderer.slide -= 0.025;
